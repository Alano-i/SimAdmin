...
```bash
curl -fsSL https://raw.githubusercontent.com/Alano-i/SimAdmin/main/install_latest.sh | sh
```

### 国内网络环境

```bash
curl -fsSL https://gh-proxy.com/https://raw.githubusercontent.com/Alano-i/SimAdmin/main/install_latest.sh | sh
```

### 可选环境变量

```bash
curl -fsSL https://raw.githubusercontent.com/Alano-i/SimAdmin/main/install_latest.sh \
  | REPO=Alano-i/SimAdmin INSTALL_DIR=/opt/simadmin SERVICE_NAME=simadmin sh
```

### 安装脚本动作说明

- 根据 `uname -m` 从 GitHub Release 下载 `simadmin-arm64.tar.gz` 或
  `simadmin-amd64.tar.gz`。
...
当前支持的 64 位 CPU 架构为 ARM64（`aarch64`/`arm64`）和 AMD64
（`x86_64`/`amd64`）。其他架构会在安装阶段明确报错。
...
```bash
curl -fsSL https://raw.githubusercontent.com/Alano-i/SimAdmin/main/uninstall.sh | sh
```

### 国内网络环境

```bash
curl -fsSL https://gh-proxy.com/https://raw.githubusercontent.com/Alano-i/SimAdmin/main/uninstall.sh | sh
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/Alano-i/SimAdmin/main/uninstall.sh \
  | sh -s -- --keep-user-data
```
